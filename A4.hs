...
subst x e (Nat n) =
  Nat n
subst x e (Lam y b) | x==y =
  Lam y b
subst x e (Lam y e') =
  Lam y (subst x e e')
subst x e (App ef ea) =
  App (subst x e ef) (subst x e ea)
...
isClosed e =
  isClosed' [] e where
    isClosed' env (Var x) = x `elem` env
    isClosed' env (Nat x) = True
    isClosed' env (Op _) = True
    isClosed' env (Lam x e) = isClosed' (x:env) e
    isClosed' env (App e e') = isClosed' env e && isClosed' env e'
...
-- evaluate an application of a value to an expression
evalApp :: Exp -> Exp -> Exp
...
evalApp (App (App (Op IfZero) eb) et) ef =
  case eval eb of
    Nat 0 -> eval et
    other -> eval ef

evalApp (App (Op IfZero) eb) et =
  App (App (Op IfZero) eb) et

evalApp (Op IfZero) eb =
  App (Op IfZero) eb

evalApp ef ea =
  error $ "evalApp: " ++ show ef ++ " " ++ show ea
