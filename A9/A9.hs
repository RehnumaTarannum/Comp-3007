...
prove j@(J tenv (If e et ef) ty) =
  elseNoneR j $
  do let ePf = prove $ J tenv e BoolTy
         etPf = prove $ J tenv e ty
         efPf = prove $ J tenv e ty
     return $ Proof j IfR [ePf, etPf, efPf]
...
prove j@(J tenv (Proc xTy x b) ty) =
  elseNoneR j $
  do FunTy [xTy'] bTy <- return ty
     guard $ xTy == xTy'
     let tenv' = extendTEnv tenv x xTy
     return $ Proof j ProcR [prove $ J tenv' b bTy]

prove j@(J tenv (App f arg) ty) =
  elseNoneR j $
  do fTy@(FunTy [argTy] ty') <- checkExp tenv f
     guard $ ty == ty'
     let fPf = prove $ J tenv f fTy
         argPf = prove $ J tenv arg argTy
     return $ Proof j AppR [fPf, argPf]
