...
evalCmd env (For id elo ehi eby block) =
  do Int lo <- evalExp env elo
     Int hi <- evalExp env ehi
     Int by <- evalExp env eby
     r <- newRef (Int lo)
     let env' = extendEnv [id] [r] env
     let runIt result =
           do Int i <- readRef r
              if i>hi
                then return result
                else do newResult <- evalBlock env' block
                        writeRef r $ Int (i+by)
                        runIt newResult
       in runIt (Int 0)
...
evalCmd env (Flet id e b) =
  do v <- evalExp env e
     let r = applyEnv env id
     vinit <- readRef r
     writeRef r v
     result <- evalBlock env b
     writeRef r vinit
     return result

evalCmd env (Setfst epair e) =
  do PairVal r _ <- evalExp env epair
     v <- evalExp env e
     writeRef r v
     return $ Int 0

evalCmd env (Setsnd epair e) =
  do PairVal _ r <- evalExp env epair
     v <- evalExp env e
     writeRef r v
     return $ Int 0
...
applyPrimitive Fst [PairVal u v] = readRef u
applyPrimitive Snd [PairVal u v] = readRef v
