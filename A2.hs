...
triplize :: [a] -> [a]
triplize [] = []
triplize (x:xs) = x : x : x : triplize xs
...
prod xs [] = []
prod (x:xs) (y:ys) =
  [(x,y)] ++ prod [x] ys ++ prod xs [y] ++ prod xs ys


-- QUESTION 6
--
-- down xs: write any function with the given type
down :: [a] -> [[a]]
down [] = []
down (x:xs) = [x] : down xs
...
prefixWords prefix = map ((++) prefix) . filter isWord
