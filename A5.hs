...
evalExp env (Cond ((t,r):clauses)) =
  if evalExp env t == Bool True
  then evalExp env r
  else evalExp env (Cond clauses)  
...
evalExp env (SLet [] [] body) =
  evalExp env body

evalExp env (SLet (id:ids) (rhs:rhss) body) =  
  evalExp
  (extendEnv [id] [evalExp env rhs] env)
  (SLet ids rhss body)
...
hasBinding env id =
  envFoldR False (\fr b -> b || hasBindingInFrame fr id) env

hasBindingInFrame :: Frame -> Id -> Bool
hasBindingInFrame frame id =
  not $ Nothing == applyFrame frame id
...
boundIds :: Env -> [Id]
boundIds = envFoldR [] (\(Frame ids _) l -> ids ++ l)

envFoldR :: a -> (Frame -> a -> a) -> Env -> a
envFoldR mt ext EmptyEnv = mt
envFoldR mt ext (ExtendedEnv fr env)
  = ext fr (envFoldR mt ext env)
envFoldR mt ext (RecExtendedEnv fr env)
  = ext fr (envFoldR mt ext env)
...
-- evaluates to 4.
--
-- Complete the evalExp clause for SLet above.
