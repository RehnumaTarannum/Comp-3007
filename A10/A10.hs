...
dft p gs =
  findSolutions $ search p 0 emptySubst gs
  where findSolutions (Solution s) = [s]
        findSolutions (Branches l) =
          concat $ map findSolutions l
...
bfs p gs =
  findSolution $ [search p 0 emptySubst gs]
  where findSolution l =
          case find isSolution l of
           Nothing -> findSolution $ concat $ map subtrees l
           Just (Solution s) -> Just s
        subtrees (Branches l) = l
        subtrees _ = []
